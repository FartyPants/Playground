...
Check out my new book "The Cranky Man's Guide to LoRA & QLoRA" 
...
# Related Projects: 
Playground is a collection of many things in one, but I also carved some of the functionality in order to refine them even further as a separate projects.

- Twinbook
[https://github.com/FartyPants/Twinbook](https://github.com/FartyPants/Twinbook)
Takes the two-notebook idea much further. It clevery combines notebook and chat. I actually use it now solely for rewriting and testing.
- Virtual Lora
...
To get latest PEFT - look in installer files:
```
text-generation-webui-new\installer_files\env\Lib\site-packages
```
find peft folder. Rename it to 
```
peft_old
```
...
and just clone the peft somewhere (not to the site_packages though as it comes with other stuff)
```
git clone https://github.com/huggingface/peft
```
or download the whole thing somewhere

You need what is in the src folder - which is subfolder peft - (that's the same thing as you just renamed to peft_old)
